...
function getNextFriday(date) {
  ...
  dateObj.setDate(dateObj.getDate() + daysUntilNextFriday);
  return dateObj;
}
...
function formatDate(date) {
  ...
  const formattedDate = `${month}/${day}/${year}, ${hours.toString()}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${ampm}`;
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = Math.floor((date - firstDayOfYear) / (24 * 3600 * 1000));
  const weekNumber = Math.ceil((diffInDays + dayOfWeek + 1) / 7);
